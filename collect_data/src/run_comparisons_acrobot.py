...
SUCCESS_PROBABILITY_CONSTRAINT = 0.6
    ...
    for x in range(20):
        ...
        for g in goals:
            for n in nodes:
                ...
                print node_name, goal_state, probability_constraint, success_constraint, random_seed
                # raw_input()
                goal_radius="goal_radius:=" + str(GOAL_RADIUS)
                subprocess.call(["roslaunch", "robust_planning", "run_comparisons_acrobot_template.launch", node_name,  total_particles, probability_constraint, success_constraint, prune_probability, prune_covariance, goal_radius, experiment_filename, mean_only, use_svm_prediction, failure_constant, random_seed])
            count = count + 1
