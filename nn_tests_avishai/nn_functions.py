...
# Building the net
def neural_net(x, weights, biases, activation_='relu'):
    # First hidden fully connected layer 
    layer = activF(tf.add(tf.matmul(x, weights['h1']), biases['b1']), activation_)

    # Remaining hidden fully connected layer 
    for i in range(1, int(len(weights)-1)):
        sth = 'h' + str(i+1)
        stb = 'b' + str(i+1)
        layer = activF(tf.add(tf.matmul(layer, weights[sth]), biases[stb]), activation_)
...
def neural_net_dropout(x, weights, biases, keep_prob, activation_='relu'):
    # First hidden fully connected layer 
    layer = activF(tf.add(tf.matmul(x, weights['h1']), biases['b1']), activation_)
    ...
    for i in range(1, int(len(weights)-1)):
        sth = 'h' + str(i+1)
        stb = 'b' + str(i+1)
        layer = activF(tf.add(tf.matmul(layer_drop, weights[sth]), biases[stb]), activation_)
        layer_drop = tf.nn.dropout(layer, keep_prob)
