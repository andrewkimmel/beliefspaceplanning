...
class DiffusionMap(BaseEstimator):
    ...
    def fit_transform(self, X, density_normalize=True):
        ...
        lambdas, V = eigsh(kernelMat, k=self.embedding_dim, tol=1E-2)  # calculate as many eigs as the requested embedding dim
    ...
    def ReducedClosestSetIndices(self, sa, X, k_manifold=10):
        ...
        V, lam = self.fit_transform(X, density_normalize=False)
        sa_r = V[0,:]
        # V = V[1:,:]
...
#     df = DiffusionMap(sigma=1, embedding_dim=2, k=500)
#     V,lam, ix = df.fit_transform(X, density_normalize=False)
